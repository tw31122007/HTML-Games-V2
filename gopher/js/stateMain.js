var StateMain = {
	...
	create: function () {
		...
		this.pickNPC = function () {
			return availNpcGophers[
				Math.floor(Math.random() * availNpcGophers.length)
			];
		};
		...
		bottomRail = game.add.tileSprite(
			0,
			385,
			this.world.width,
			47,
		...
		//COUNTDOWN SPRITES
		this.countDown1 = game.add.sprite(
			game.world.bounds.height - 135,
			game.world.centerY,
			'countDown1'
		);
		...
		this.countDown2 = game.add.sprite(
			game.world.bounds.height - 135,
			game.world.centerY,
			'countDown2'
		);
		...
		this.countDown3 = game.add.sprite(
			game.world.bounds.height - 135,
			game.world.centerY,
			'countDown3'
		);
		...
		this.countDownGo = game.add.sprite(
			game.world.bounds.height - 80,
			game.world.centerY,
			'countDownGo'
		);
		...
		//TWEENS for 3..2..1..GO!
		var tween1 = game.add
			.tween(this.countDown1)
			.to({ alpha: 1 }, 500, Phaser.Easing.Linear.None, false, 0)
			.to({ alpha: 0 }, 500, Phaser.Easing.Linear.None, false, 0);
		var tween2 = game.add
			.tween(this.countDown2)
			.to({ alpha: 1 }, 500, Phaser.Easing.Linear.None, false, 0)
			.to({ alpha: 0 }, 500, Phaser.Easing.Linear.None, false, 0);
		var tween3 = game.add
			.tween(this.countDown3)
			.to({ alpha: 1 }, 500, Phaser.Easing.Linear.None, false, 0)
			.to({ alpha: 0 }, 500, Phaser.Easing.Linear.None, false, 0);
		var tweenGo = game.add
			.tween(this.countDownGo)
			.to({ alpha: 1 }, 500, Phaser.Easing.Linear.None, false, 0)
			.to({ alpha: 0 }, 500, Phaser.Easing.Linear.None, false, 0);
		tween3.chain(tween2);
		tween2.chain(tween1);
		...
		this.timer.fixedToCamera =
			scoreText.fixedToCamera =
			timeLabel.fixedToCamera =
			timeText.fixedToCamera =
			scoreLabel.fixedToCamera =
				true;
	...
	setListeners: function () {
		...
		game.time.events.loop(
			Phaser.Timer.SECOND * coinSpawnRate,
			this.loadCoin,
			this
		);
		game.time.events.loop(
			Phaser.Timer.SECOND * npcSpawnRate,
			this.loadNPC,
			this
		);
	},
