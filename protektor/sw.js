...
function IsUrlInLazyLoadList(url, lazyLoadList) {
  if (!lazyLoadList) return false; // presumably lazy load list failed to load
...
function WriteLazyLoadListToStorage(lazyLoadList) {
  ...
  // bypass if localforage not imported
...
async function UpdateCheck(isFirst) {
  try {
    ...
    if (lazyLoadList) await WriteLazyLoadListToStorage(lazyLoadList); // dump lazy load list to local storage#
...
async function HandleFetch(event, doUpdateCheck) {
  ...
  if (cachedResponse) return cachedResponse; // use cached response
...
self.addEventListener("fetch", (event) => {
  ...
  if (doUpdateCheck) {
    ...
    event.waitUntil(
      responsePromise.then(() => UpdateCheck(false)) // not first check
    );
  }
